...
#include <cstring>
#ifdef application_commands_cpp
...
setStatus(
  ...
  )
  {
        ...
        if (argc < 2)
                {
                if(statusmsg == 1)
                  pThis->printf("Confirmed Packet\n");
                else
                  pThis->printf("Un-Confirmed Packet\n");
                return cCommandStream::CommandStatus::kSuccess;
                }
        else
                {
                ...
                // get arg 1 as tx period; default is irrelevant because
                // we know that we have argv[1] from above.
                status = getuint32(argc, argv, 1, packStatus, 0);

                if (status != cCommandStream::CommandStatus::kSuccess)
                        return status;

                if((packStatus != 0) && (packStatus != 1)){
                  pThis->printf("Incorrect Packet Status\n");
                }else{
                  statusmsg = packStatus;
                  fram.write8(0x50,statusmsg);
                }    
                return status;
                }
        }
