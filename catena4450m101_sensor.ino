...
#define application_commands_cpp
#include <cmath>
#include <cstring>
#include <type_traits>
...
extern cCommandStream::CommandFn setStatus;
uint8_t statusmsg;
uint8_t msgStat;
...
void setup(void)
        ...
        //gCatena.SafePrintf("(%p)", pPlatform);

       if(fram.begin(0x50)){
          Serial.println("FRAM Initialised");
          msgStat = fram.read8(0x50);
          
          if((msgStat != 0) && (msgStat != 1)){
             msgStat = 0;
          }
       }else{
        ...
        if (flags & ThisCatena::fHasLuxRohm)
               ...
               // Serial.println(/*flags & ThisCatena::fHasLuxRohm*/);
...
void startSendingUplink(void)
{
  ...
  *pFlag = uint8_t(flag);
  ...
  gLoRaWAN.SendBuffer(b.getbase(), b.getn(), sendBufferDoneCb, NULL, msgStat);
  //gCatena.SafePrintf("%p\n", statusmsg);
}
